...
- **All Canvas IDs filled in (2026-06-25):** `COURSE.canvasId` confirmed "920". All 25 `CHAPTERS.all[*].fileId` filled from the `hanley-chapters` Canvas Files folder (folder id 23058) via `GET /api/v1/folders/23058/files`. All 8 `GUIDED_NOTES[*].aid` filled in as 9067-9074 — confirmed correct via `GET /api/v1/courses/920/assignments?per_page=100`, which shows them explicitly named "Guided Notes 1" through "Guided Notes 8" in that exact order (and confirms `points_possible: 0.0`, same `assignment_group_id` as Puzzles — matches the ECO 1000 bonus mechanic exactly). All 8 `CANVAS.puzzles[*].aid` filled in from the same assignments call: 7768, 7936, 8762, 8116, 8767, 8768, 8769, 8770 (Puzzle 1→8). Files are flagged `hidden: true` in Canvas — expected for link-only distribution, but worth a one-time student-view sanity check that the links actually load.
- **chapters.html refinement (2026-06-25):** Guided Notes pill moved from every chapter in a block to just the FIRST chapter (`item.notesStartLabel` in config.js) — pairs with the existing Submit-on-last-chapter behavior, so each multi-chapter document now reads as "notes start here → ... → submit once here" instead of repeating either pill on every row.
