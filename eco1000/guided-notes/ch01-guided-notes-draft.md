# ECO 1000 | Guided Notes — Ch. 1: What Is Economics?

**Name:** ________________________  **Section:** ________  **Date:** ________

---

## By the end of this lecture, I should be able to:

1. Distinguish scarcity from poverty and explain why scarcity is permanent even if poverty is not.
2. Apply at least three of the eight guideposts to a real decision, identifying the opportunity cost and incentive structure involved.
3. Classify an economic statement as positive or normative and name the reasoning pitfall present in a flawed economic argument.

---

## Key Terms & Scaffolded Definitions

| Term | Pre-filled frame | Complete: ___ |
|---|---|---|
| Scarcity | Scarcity exists when there is _____ of a resource freely available than people _____. | |
| Economic good | A good is an economic good when it is _____. | |
| Resources | The _____ people use to produce goods and services; may be human, _____, or natural. | |
| Rationing | The method by which society _____ scarce resources among competing _____. | |
| Opportunity cost | The _____ alternative you give up when you make a choice. | |
| Economizing | Getting a specific _____ at the _____ possible cost. | |
| Marginal analysis | Comparing _____ costs to _____ benefits to make a decision. | |
| Ceteris paribus | Latin for "_____" — holds _____ constant while examining one change. | |
| Positive economics | Studies "what _____" — statements that can be proved _____ or _____. | |
| Normative economics | Makes judgments about "what _____" — statements that _____ be proved true or false. | |
| Fallacy of composition | Assuming what is true for the _____ is also true for the _____. | |

---

## Section I — What Is Economics About?

*Cornell two-column — causal chain: scarcity → choice → rationing → competition*

**Connection to prior learning:**
This is lecture one — no prior economics assumed. Scarcity is the foundation every other concept this semester builds on.

| Questions | Notes |
|---|---|
| Scarcity and poverty both involve not having enough — so why do economists insist they're different, and why does the distinction matter? | Scarcity = _____ concept: limited resources, unlimited desires. Poverty = _____ concept; people disagree on what "poor" means. Poverty _____ be eliminated someday; scarcity _______. |
| You're on the committee that allocates transplant organs in the U.S. What criteria could you use to ration them? Which does the market rely on — and what does that imply about who gets the organ? | Rationing methods: first-come/first-served; _____, need, ability to pay (price). Market uses _____. Allocated to those willing to give up the most _____ to obtain ownership. |
| New York City replaced free parking with paid meters in several neighborhoods. How did that change the *form* of competition for a parking spot without eliminating competition? | Competition is a natural outgrowth of _____. Change rationing → change the _____ of competition, but NOT _____. Example from NYC: _______ |

---

## Section II — The Eight Guideposts to Economic Thinking

*Comparison matrix — 8 rows*

**Connection to Section I:**
Scarcity forces choice. The eight guideposts tell you *how* economists think through those choices.

| # | Guidepost | Core idea (fill in) | You're the decision-maker: apply it |
|---|---|---|---|
| 1 | Trade-offs | Using scarce resources always has an _____ cost — the _____ alternative foregone. | You're a hospital budget director. You spend $2M on a new MRI machine. What's the opportunity cost? _______ |
| 2 | Purposeful choice | People _____ — they try to get the _____ from limited resources. This is called _____. | You're hiring for a role that pays $60K or $80K. How does purposeful choice predict who applies? _______ |
| 3 | Incentives | ↑ benefit of an action → _____ likely to do it. ↑ cost → _____. Both _____ and _____ incentives matter. | Uber raised surge pricing during a snowstorm in Boston. What happened to driver supply? Why? _______ |
| 4 | Marginal thinking | Compare _____ benefits to _____ costs. Act when _____ > _____. | Amazon's fulfillment center is deciding whether to hire a 101st worker. What's the right question to ask? _______ |
| 5 | Costly information | Better information helps, but acquiring it is _____. Stop gathering info when its _____ equals its _____. | You're a buyer at Target deciding how long to test a new product before a nationwide rollout. When do you stop? _______ |
| 6 | Secondary effects | Actions generate _____ effects in addition to the intended _____ effects. | When San Francisco raised its minimum wage, what secondary effect on employment or hours would you predict? _______ |
| 7 | Subjective value | Value is _____ — the same good is worth different amounts to different _____. | A Taylor Swift concert ticket resells for $800 above face value. What does guidepost 7 explain? _______ |
| 8 | Testability | A theory's value is its ability to _____ behavior. | An economist claims that raising interest rates reduces inflation. How would you test whether that's true? _______ |

**Synthesis — Guideposts 3 & 6 + Cross-guidepost tension:**

New York City's rent stabilization law caps rent increases for covered apartments. You're a housing policy analyst advising the city.

Using guidepost 3 (incentives) and guidepost 6 (secondary effects), what two consequences do you predict for the housing market?

Consequence 1 (incentives): _________________________________________

Consequence 2 (secondary effects): _________________________________________

**Cross-guidepost question:** Two of the eight guideposts are in tension in this rent-stabilization scenario. Identify which two, explain the tension, and decide which matters more for a housing policy analyst.

_________________________________________

---

## Section III — Positive vs. Normative Economics

*Comparison matrix*

**Connection to Sections I–II:**
The guideposts describe how economists analyze. Positive vs. normative describes *what kind of question* an economist can answer.

| | Positive | Normative |
|---|---|---|
| Asks… | "What _____?" | "What _____ to be?" |
| Can be tested? | _____: can be proved _____ or _____ | _____: reflects _____ judgments |
| Chapter example | _______________________ | _______________________ |
| Your own example | _______________________ | _______________________ |
| Why it matters in policy debates | | |
| **Classify this:** "When the Fed raises interest rates, inflation falls." | Positive / Normative? _____ Justify: _________________________ | |
| **Classify this:** "The Fed should keep interest rates low to support job growth." | Positive / Normative? _____ Justify: _________________________ | |

---

## Section IV — Pitfalls to Avoid in Economic Thinking

*Comparison matrix — 4 pitfalls*

**Connection to Sections I–III:**
Even careful reasoning goes wrong. These four pitfalls are the most common errors in news coverage, textbooks, and policy arguments.

| Pitfall | The error | Pre-filled example | How to avoid it (you fill in) |
|---|---|---|---|
| Ceteris paribus violation | Failing to hold _____ constant when isolating one _____ | "Cities that build more highways have worse traffic — so highway construction causes congestion." What's wrong? | |
| Good intentions ≠ good outcomes | _____ intentions don't guarantee _____ outcomes; watch for _____ secondary effects | A city bans plastic bags to reduce waste. Shoppers switch to reusable bags — but reusable bags require far more resources to produce. What went wrong? | |
| Association ≠ causation | Statistical _____ alone does not establish _____ | Ice cream sales and drowning deaths both rise in summer. Does ice cream cause drowning? | |
| Fallacy of composition | What is true for the _____ may not be true for the _____ | Each fan at a sold-out stadium can see better by standing up. So if everyone stands up, everyone sees better. True or false — and which pitfall is this? | |

---

## Self-Test: Cover & Recite

Cover your Notes column. Use only your Questions column and the row headers from each matrix.

- Can you explain each answer in your own words — without looking?
- Can you give your own example for each of the eight guideposts?
- Can you explain the difference between positive and normative without looking at the matrix?

Uncover and compare. **Mark anything incomplete or wrong with a star ★.**

---

## In My Own Words — Lecture Summary

*(Complete this before you close your notes.)*

The main idea of today's lecture was: ________________________________

The concept I found hardest to understand was: _______________________
because: ___________________________________________________________

One real-world situation this made me think of: _______________________

---

## Reflect & Recapitulate

You're a policy analyst advising North Carolina's housing agency. The governor wants to freeze rent increases to address affordability. Using at least three concepts from today — scarcity, rationing, opportunity cost, incentives, secondary effects, positive vs. normative — draft the opening of a one-paragraph briefing: what does the economic analysis say (positive) and what would you recommend (normative)?

_________________________________________________

_________________________________________________

_________________________________________________

Which of the four reasoning pitfalls is most likely to trap a policymaker who supports this plan without doing the economic analysis? Explain.

_________________________________________________

_________________________________________________

---

*Submit your completed notes to Canvas by [Date] for **2 participation points**.*
*Notes will be checked for completion — summary box and Self-Test marks required.*

**STUDY TIP:** Don't re-read these notes passively.
Cover the Notes column. Use your Questions column and matrix row headers to quiz yourself — out loud. Repeat daily until the quiz.
